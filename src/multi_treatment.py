...
random_seed = 42
n_users = 1000000
coupon_settings = {"A": 1000, "B": 1500, "C": 2000}
...
def assign_coupon_by_score(
    ...
    # クーポン予算制限あり
    coupon_budgets = {
        "A": len(df_test) - (len(df_test) // 3 + len(df_test) // 3),
        "B": len(df_test) // 3,
        "C": len(df_test) // 3,
    }
    ...
    df_test_score[f"{score_name}_C-A"] = (
        df_test_score[f"{cvr_name}_C"] - df_test_score[f"{cvr_name}_A"]
    ) / (coupon_settings["C"] - coupon_settings["A"])

    df_test_score = pd.melt(
        df_test_score,
        id_vars=["user_id"],
        value_vars=[f"{score_name}_B-A", f"{score_name}_C-A"],
        var_name="score_type",
        value_name="score",
    )
...
def calc_cumulative_uplift(
    ...
    baseline_cv = df_test.apply(
        lambda row: row["cv"] if row["coupon"] == "B" else np.nan,
        axis=1,
    )
