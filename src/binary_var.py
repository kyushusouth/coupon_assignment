...
def main():
    ...
    shap_values_test = explainer.shap_values(
        df_test[num_cols + cat_cols].sample(min(len(df_test), 1000))
    )
    plt.figure(figsize=(6, 10))
    shap.summary_plot(
        shap_values_test, df_test[num_cols + cat_cols], plot_type="bar", show=False
    )
    plt.tight_layout()
    ...
    plt.figure(figsize=(6, 10))
    shap.summary_plot(
        shap_values_test, df_test[num_cols + cat_cols], plot_type="dot", show=False
    )
    plt.tight_layout()
    ...
    proba_map = df_res.groupby(["user_index", "coupon"])["proba"].first().to_dict()
    cost_map = df_res.groupby(["user_index", "coupon"])["cost"].first().to_dict()

    # 機械学習モデルによって取得されたcv確率をもとに、最適なクーポンの割り当て方を求める

    user_indices = df_res["user_index"].unique().tolist()
    problem = pulp.LpProblem("Coupon_Optimization", pulp.LpMaximize)
    x = pulp.LpVariable.dicts("assign", (user_indices, coupon_types), cat=pulp.LpBinary)

    problem += pulp.lpSum(
        proba_map[(user_index, coupon_name)] * x[user_index][coupon_name]
    ...
    for user_index in user_indices:
        problem += (
            pulp.lpSum(x[user_index][coupon_name] for coupon_name in coupon_types) == 1
        )

    expected_cost = pulp.lpSum(
        cost_map[(user_index, coupon_name)] * x[user_index][coupon_name]
        for user_index in user_indices
        for coupon_name in coupon_types
    )
