...
class Trainer:
    def __init__(self):
        ...
        self.coupon_map = {"A": 1000, "B": 1500, "C": 2000}
...
def solve_cost_minimization_problem(
    ...
    # 制約条件2: 各セグメントにすべてのクーポンを1%以上割り当てる
    for s in segments:
        for c in coupon_types:
            prob += x[s][c] >= 0.01
...
def solve_cv_maximization_problem(
    ...
    # 制約条件3: 各セグメントにすべてのクーポンを1%以上割り当てる
    for s in segments:
        for c in coupon_types:
            prob += x[s][c] >= 0.01
...
def main():
    ...
    test_pred_df["cvr_rank_C"] = pd.qcut(
        test_pred_df["pred_cvr_C"], 10, duplicates="drop"
    )
    ...
    # 制約条件下で達成可能なコストの最小値を取得
    min_cost = solve_cost_minimization_problem(segment_df, trainer.coupon_map)
    ...
    results = []
    for budget in sorted(list(range(min_cost, 50000000, 1000000)) + [estimated_cost_B]):
        if not budget == estimated_cost_B:
            continue

        assign_solution = solve_cv_maximization_problem(
            segment_df, trainer.coupon_map, budget
        )
